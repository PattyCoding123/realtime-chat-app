...
const ChatInput: FC<ChatInputProps> = ({ receiverUser, chatId }) => {
  const textareaRef = useRef<HTMLTextAreaElement | null>(null);
  const [isLoading, setIsLoading] = useState(false); // Loading state
  const [input, setInput] = useState(""); // Text area controlled
    ...
    <div className="mb-2 border-t border-gray-200 px-4 pt-4 sm:mb-0">
      <div className="relative flex-1 overflow-hidden rounded-lg shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-indigo-600">
        <TextAreaAutosize
          ...
          placeholder={`Message ${receiverUser.firstName} ${receiverUser.lastName}`}
